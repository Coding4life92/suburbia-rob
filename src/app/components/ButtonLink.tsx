...
export function ButtonLink({
    ...
    return (
        <PrismicNextLink
            className={clsx(
                "button-cutout group mx-4 inline-flex items-center font-bold transition-[filter,background-position] duration-300 bg-gradient-to-b from-25% to-75% bg-[length:100%_400%] hover:bg-bottom rounded-md",
                // Sizes
                size === "sm" && "gap-2.5 py-2 px-3 text-base",
                size === "md" && "gap-3 py-[clamp(0.625rem,2vw,0.75rem)] px-4 text-lg",
                size === "lg" && "gap-4 py-[clamp(0.75rem,2.5vw,1rem)] px-6 text-xl",
                // Colors
                ...
                <>
                    <div
                        className={clsx(
                            ...
                            size === "md" && "h-5 w-5 sm:h-6 sm:w-6",
                            size === "lg" && "h-6 w-6 sm:h-8 sm:w-8",
                    ...
                    <div className="w-px self-stretch bg-black/25 mx-2" />
                </>
            ) : null}
            {children}
        </PrismicNextLink>
