    ...
    return (
        <div className={clsx('flex flex-col gap-6', className)}>
            <Options title='Deck' selectedName={selectedDeck?.uid}>
                {decks.map((deck) => (
                    <Option
                        key={deck.uid} imageField={deck.texture}
                        imgixParams={{
            ...
            <Options title='Wheels'>
                {wheels.map((wheel) => (
                    <Option
                        key={wheel.uid} imageField={wheel.texture}
                        imgixParams={{
                            rect: [20, 1550, 1000, 1000],
                            width: 150,
                            height: 150
            ...
            <Options title='Trucks'>
                {metals.map((metal) => (
                    <Option
                        key={metal.uid} colorField={metal.color}
...
function Option({ children, selected, imageField, imgixParams, colorField, onClick }: OptionProps) {
    return (
        <li>
            <button
                className={clsx('size-10 cursor-pointer rounded-full bg-black p-0.5',
            ...
            >
                {imageField ? (
                    <PrismicNextImage
                        ...
                        className="pounter-events-none h-full w-full rounded-full"
                        alt=""
                    />
                ) : (
                    <div
